...
##prop mellif
plantt <- read.csv ( "plantetoutstation.csv",header = T, sep = ";")
planttmel <- select (plantt,nom,Mellifere,nom_latin )
planttmel <- distinct(planttmel)
planttmel <- planttmel %>% group_by(nom) %>% group_by(nom_latin) %>% count(nom, Mellifere)
planttmel<- planttmel%>% group_by(nom) %>% count(nom, Mellifere)

plantttmel <- planttmel %>% group_by(nom) %>% mutate(prop = n/ sum(n) *100) 
plantttmel <-plantttmel %>% mutate_if(is.numeric, ~round(., 1))

planttoutFA <- plantttmel %>% group_by(nom) %>% filter(Mellifere == "FALSE")
planttoutFA <- rename(planttoutFA, c(nbr_pas_mell= n, prop_pas_mell = prop) )
planttoutTR <- plantttmel %>% group_by(nom) %>% filter(Mellifere == "TRUE")
planttoutTR <- rename(planttoutTR, c(nbr_mell= n, prop_mell = prop) )

planttoutmel <- full_join(planttoutFA ,planttoutTR, by = "nom")
planttoutmel <- subset (planttoutmel, select = -c(Mellifere.x,Mellifere.y))
names(planttoutmel) <- c("nom","nbr_plant_non_mel","prop_plant_non_mel","nbr_plant_mel","prop_plant_mel")

##prop indi

plantt <-read.csv("plantetoutstation.csv",header = T, sep = ";")
planttti <- select (plantt,nom,Indigene,nom_latin )
planttti <- planttti %>% group_by(nom) %>% group_by(nom_latin)
plantttinbr <- planttti %>% group_by(nom) %>% group_by(nom_latin) %>% count(nom, Indigene)
plantttinbr<- plantttinbr%>% group_by(nom) %>% count(nom, Indigene)

plantttinbr <- plantttinbr %>% group_by(nom) %>% mutate(prop = n/ sum(n) *100) 
plantttinbr <-plantttinbr %>% mutate_if(is.numeric, ~round(., 1))

plantttFA <- plantttinbr %>% group_by(nom) %>% filter(Indigene == "FALSE")
plantttFA <- rename(plantttFA, c(nbr_pas_ind= n, prop_pas_ind = prop) )
plantttTR <- plantttinbr %>% group_by(nom) %>% filter(Indigene == "TRUE")
...
planttoutind <- subset (planttoutind, select = -c(Indigene.x,Indigene.y))
