...
Si l'on avait continuer a échantillonné, on aurait selon Chao collecté
111.6 espèces avec une SE de 12.898. Par défaut 100 permutation. En
regardant l'ensemble des estimateurs de richesses, on aurait obtenu
entre 98 et 121 espèces.

Des explications sont disponibles sur :
<https://www.youtube.com/watch?v=I-_PjHT5nB4>

D'après McIntyre (vidéo YT), specpool() sert à calculer les estimateurs
pour les incidence data. EstimateR() lui sert à calculer les abundance
data. La fonction specpoool() du package vegan calcul tous les
estimateurs. Il ne donne pas Chao1 et Chao2 mais juste Chao pour formule
: Chao S_P = S_0 + a1\^2/(2*a2)* (N-1)/N Chao bias-corrected S_P = S_0 +
a1*(a1-1)/(2*(a2+1)) \* (N-1)/N
...
Le package fossil permet également de calculer la richesse avec les
différents estimateurs. Les résultats ne sont pas les même qu'avec
specpool(), peut-être que les formules utilisées sont légèrement
différentes ? Chao1 est censé être l'estimateur le plus adapté, mais on
voit qu'il donne un SD énorme (21). Cela se voit également lorsque l'on
regarde le graphique de Chao, il a un CI très large et de forme
particulière. Comment l'expliquer ?
...
Information sur estimateR : 2021
<https://cran.r-project.org/web/packages/vegan/vignettes/diversity-vegan.pdf>
2022 (plus complet)
<https://cran.r-project.org/web/packages/vegan/vegan.pdf> EstimateR()
semble calculer la richesse pour chacun des sites et en même temps il a
besoin de données d'abondance. Par contre, specpool() semble être
utilisé pour l'ensemble des sites et nécessiterai seulement des données
d'incidence (présence-absence ou donnée imprécise). ?

Dans ?estimateR(), il différencie poolacc() et estimateR() en modèle
d'accumulation et modèle quantitative estimateR() semble être similaire
à specpool() tout comme l'est poolaccum() et estaccumR sont similaire à
specaccum() =\> utilisé pour la courbe d'accumulation.
...
C'est étrange, on dirait que estaccumR() utilise des abundance-based,
car il utilise Chao qui semble différent de celui utilisait avec
poolaccu() (=Chao1?). La documentation dit qu'il donne chaque fois le
Chao biaisé Pourtant la documentation semble indiquer que tout comme
specpool() il utilise des incidence-based. estacumR() calcul sur
l'ensemble des sites des estimateurs abundance-based, Chao est semblable
à Chao1 de spp.est() et McIntyre dit la même chose.
...
veiledspec() estime à 97.7 sp sur 89 obs, soit 8.7 espèces rares non
découvertes sur l'ensemble des sites.
