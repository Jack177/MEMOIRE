...
#Isolation du chunk
bee <- data_bee_sp
# Nombre d'espèces observées dans les 15 Site
(bee.nbsp <- specnumber(bee))
# Site avec une richesse en espèces minimale et maximale observée
bee.nbsp[bee.nbsp == min(bee.nbsp)]
bee.nbsp[bee.nbsp == max(bee.nbsp)]
range(bee.nbsp)
# Abondance totale dans les 15 Site
(bee.abund <- rowSums(bee))
range(bee.abund)
# Abondance dans les Site avec le plus petit nombre d'espèces
bee.abund[bee.nbsp == min(bee.nbsp)]
# Abondance dans les Site avec le plus grand nombre d'espèces
bee.abund[bee.nbsp == max(bee.nbsp)]
# Nombre d'espèces dans les Site avec la plus petite abondance
bee.nbsp[bee.abund == min(bee.abund)]
# Nombre d'espèces dans les Site avec la plus grande abondance
...
bee.rare39 <- rarefy(bee, sample = 39)
# Comparer le classement de la richesse entre les Site observés et raréfiés
sort(bee.nbsp)
sort(round(bee.rare39)) 
# Site avec une richesse en espèces minimale et maximale estimée
bee.rare39[bee.rare39 == min(bee.rare39)]
bee.rare39[bee.rare39 == max(bee.rare39)]
# Site observé avec la plus petite richesse prédite en espèces
bee[which(bee.rare39 == min(bee.rare39)),]
# Site observé avec la plus grande richesse d'espèces prédite
bee[which(bee.rare39 == max(bee.rare39)),]
...
#data(BCI)
out <- rarecurve(bee, step = 1, sample = 39, label = TRUE, col = "blue")
...
ggplot(xy, aes(x = subsample, y = value, color = species)) +
    ...
    geom_line() +
  geom_vline(xintercept = 39, color = "gray") +
...
graphhill <- tibble::rownames_to_column(graphhill, "Site")

graphhill<-reshape::melt(graphhill)
graphhill<-graphhill %>%group_by(Site)

nb.cols <- 15
mycolors <- colorRampPalette(brewer.pal(8, "Set1"))(nb.cols)

ggplot(graphhill,aes(x = variable, y= value, group = Site, color = Site))   +
  geom_line()+ 
  geom_point()+
  xlab("Nombre de Hill") +
  ylab("Richesse observées")  +
  guides(fill = guide_colourbar(title = "")) + # Titre légende
  scale_y_continuous(breaks = round(seq(min(0), max(200), by = 4),1))+
  labs(
    title = "",
    subtitle = "",
    caption = "") +
  #guides(fill = guide_legend(title = "Site")) + # Titre légende (marche slmt pour Heatmap)
...
data (doubs)
spe <- doubs$fish# Une rivi?re, 30 Site, une longue liste de poissons
env <- doubs$env # La m?me rivi?re, les m?mes Site, mais de nombreux types de descripteurs environnementaux
?doubs #Plus d'infos
...
```{r}
# Nombre d'espèces dans les 15 Site
