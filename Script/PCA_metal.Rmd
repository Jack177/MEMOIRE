...
### Sol
# Metaux
soil_site_name <- select(soil, code_site)
metal <- soil[, c("As", "Cd", "Cr", "Cu", "Hg", "Pb", "Ni", "Zn", "Code")]
row.names(metal) <- soil_site_name$code_site
...
pca_m1 <- rda(m, scaling = TRUE)

# Centrer et réduire 
#scale (m, center = TRUE, scale = TRUE)

# Calcul PCA standardized avec vegan::prcomp() ACP normée (centré et réduite)
pca_m2 <- prcomp(m, center = TRUE, scale = TRUE)

# rda() donne plus d'information, score sp et site
summary(pca_m1)
s <- summary(pca_m2)
screeplot(pca_m2, bstick = TRUE)


biplot(pca_m2, scaling = "sites")
autoplot(pca_m2, data = metal, colour = "Code", label = TRUE, label.size = 3, shape = FALSE,
