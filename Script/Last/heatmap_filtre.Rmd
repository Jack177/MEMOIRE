...
```{r}
#data <- filter(SCS, Nest_area == "Soil")
```
...
ggplot(hm.bee_sp, aes(x = Site, y = sp, fill = nbr)) +
  labs(title = "M", x = '', y = '') + # Titre et titre axe
  geom_tile(color = "black") + #couleur grille
  coord_fixed() +
  #coord_flip() + # 180°
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0, size = 5, hjust = 0, colour = mycolors),
        ...
        plot.title = element_text(size = 28, hjust = 0.5, colour = "white"), # Taille titre texte 
        legend.key.size = unit(.25, "cm"),
        legend.key.width = unit(.25,"cm"),) + # taille barre légende) + #transparent legend panel
...
# dev.off()
#ggsave(filename = "D:/OneDrive - UMONS/UMONS/MA2/MEMOIRE/Statistique/R/STAT-MEMOIRE/Output/heatmap_terricole.png", bg='transparent')
