...
ggplot(bee_sp, aes(x = sp)) +
  ...
  annotation_custom(grobTree(textGrob("Doubleton", x=0.78, y=0.8, hjust=0, gp=gpar(col="dark red", fontsize=15)))) + # texte doubleton
  geom_vline(xintercept = "Andrena nigroaenea", colour = "dark red", size=1.1) + # ligne vertical singleton
  annotation_custom(grobTree(textGrob("Singleton", x=0.9,  y=0.8, hjust=0, gp=gpar(col="dark red", fontsize=15)))) + # texte singleton
  theme(axis.title.x = element_text(face="bold", size = 12), # Titre axe x
        axis.title.y = element_text(face="bold", size = 12), # Titre axe y
        axis.title.y.right = element_text(angle = 90), # Angle 2ème axe y
        axis.text.x = element_text(face="bold.italic", color= "black", size=10, angle=90, hjust=0), # Echelle x
